...
```{julia}
# New spatial resolution (100m pixels)
size_pixel_new = 100 

# Each new pixel contains 100×100 = 10,000 original pixels
...
axlist = [
    ...
    Dim{:Classes}(["class$i" for i in 1:n_classes])
]
...
```{julia}
axlist_lst = [
    Dim{:x}(1:size(new_res_array_lst, 1)),
    Dim{:y}(1:size(new_res_array_lst, 2))
]
...
```{julia}
# Define parameters
pft_list = ["class1", "class2", "class3"]
winsize = 5      # Moving window size
minpxl = 10      # Minimum pixels required
minDiffPxls = 0.4  # Minimum difference in pixel percentages
...
results = space4time_proc(
    ...
    winsize = winsize,
    minpxl = minpxl,
    minDiffPxlspercentage = minDiffPxls
)
```
