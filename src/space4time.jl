...
function s4time(
    ...
    if isnothing(time_n)
        ...
        if count(!isnan, climvarmat[:]) >= minpxl
            ...
            # altitude processing
            altitude_center_1 =
                altitude_cube_in[Int(round(winsize/2)+1), Int(round(winsize/2)+1), 1]
            altitude_center_2 =
                altitude_cube_in[Int(round(winsize/2)+1), Int(round(winsize/2)+1), 2]
            ...
            if isfinite(sum(pftsvarmat)) && sum(sum(pftsvarmat, dims = 1) .> 0.0) > 1
                #println("test")
                ...
                if uniquepixels >= minDiffPxls && sum(pftpres_check) > 1
                    ...
                    lc1 =
                        map(x -> x / (sum(x) + 0.000001), eachslice(pftsvarmat_f, dims = 1))
                    lc1 = reduce(vcat, lc1')
                    ...
                    if sum(lcsvd.S) > 0
                        ...
                        uniquepixels_char_altitude_1 =
                            unique(round.(altitude_1, digits = 6))
                        uniquepixels_char_altitude_2 =
                            unique(round.(altitude_2, digits = 6))
                        ...
                        if uniquepixels_altitude_1 >= minDiffPxls_alt &&
                           uniquepixels_altitude_2 >= minDiffPxls_alt
                            ols = lm(
                                [ones(size(lr, 1)) lr altitude_1 altitude_2],
                                identity.(climvarmat[:]);
                                method = :qr,
                                dropcollinear = false,
                            )
                        ...
                        if isfinite(sum(coef_reg))
                            ...
                            if isa(bogusc2, Vector)
                                if n_altitude == 4
                                    boguspred = predict(
                                        ols,
                                        [ones(length(bogusc3)) bogusc3 zeros(
                                            length(bogusc3),
                                        ) zeros(length(bogusc3))],
                                    )
                                else
                                ...
                                if n_altitude == 4
                                    boguspred = predict(
                                        ols,
                                        [ones(size(bogusc3, 1)) bogusc3 zeros(
                                            size(bogusc3, 1),
                                        ) zeros(size(bogusc3, 1))],
                                    )
                                else
                                    if n_altitude == 2 || n_altitude == 3
                                        boguspred = predict(
                            ...
                            if n_altitude == 4
                                x2pred = [ones(size(bogusc3, 1), 1) bogusc3 zeros(
                                    size(bogusc3, 1),
                                    1,
                                ) zeros(size(bogusc3, 1), 1)]
                            ...
                            if length(diff_clim_pft_pred) == 1
                                ...
                                out_3[1:transitions_n, 2] = sqrt.(diff_clim_pft_pred_var)
        ...
        if time_n == 1
            ...
            pfts_cube_in_1 = permutedims(
                reshape(pfts_cube_in_1, (winsize, winsize, nc, 1)),
                (4, 1, 2, 3),
            )
        ...
        for it = 1:time_n
            ...
            if count(!isnan, climvarmat[:, it]) >= minpxl
                ...
                # altitude processing
                altitude_center_1 =
                    altitude_cube_in[round(winsize/2)+1, round(winsize/2)+1, 1]
                altitude_center_2 =
                    altitude_cube_in[round(winsize/2)+1, round(winsize/2)+1, 2]

                altitude_1 =
                    altitude_center_1 .- reshape(altitude_cube_in[:, :, 1], winsize^2)
                altitude_2 =
                    altitude_center_2 .- reshape(altitude_cube_in[:, :, 2], winsize^2)
                ...
                if isfinite(sum(pftsvarmat)) && sum(sum(pftsvarmat, dims = 1) .> 0.0) > 1
                    ...
                    if uniquepixels >= minDiffPxls && sum(pftpres_check) > 1
                        ...
                        if sum(lcsvd.S) > 0
                            ...
                            temp = round.(
                                cumsum(lcsvd.S .^ 2) ./ sum(lcsvd.S .^ 2),
                                digits = 8,
                            )
                            ndim = minimum(findall(temp .>= 1))
                            ...
                            if isfinite(sum(coef_reg))
                                ...
                                diff_clim_pft_pred =
                                    round.((predres .- predres'), digits = 10)
                                ...
                                if length(diff_clim_pft_pred) == 1
                                    ...
                                    #else
                                    diff_clim_pft_pred_var[diff_clim_pft_pred_var .< 0] .=
                                        NaN
    ...
    if !isnothing(time_axis_name)
        ...
        out_1_dims = OutDims(
            ...
            Dim{:summary_stat}([
                ...
                "cumulative_variance",
                "predicted",
                "p-val_alt_"*altitude_vec[1],
                "p-val_alt_"*altitude_vec[2],
                "aic",
                "aicc",
                "model_used",
            ]),
        ...
        out_3_dims = OutDims(
            Dim{time_axis_name}(time_seq),
            Dim{:transitions}([
                join(pftstrans_comb_names[i], " to ") for
                i in eachindex(pftstrans_comb_names)
            ]),
        ...
        out_1_dims = OutDims(
            Dim{:summary_stat}([
                ...
                "cumulative_variance",
                "predicted",
                "p-val_"*altitude_vec[1],
                "p-val_"*altitude_vec[2],
                "aic",
                "aicc",
                "model_used",
            ]),
                    ...
                    if sum(lcsvd.S) > 0
                        ...
                        if isfinite(sum(coef_reg))
                            ...
                            if isa(bogusc2, Vector)
                                ...
                                boguspred = predict(ols, [ones(length(bogusc3)) bogusc3])
                                # boguspred = GLM.predict(compreg, DataFrame( x1 = bogusc3))

                            else
                                # boguspred = GLM.predict(compreg, DataFrame(bogusc3, :auto))
                                boguspred = predict(ols, [ones(size(bogusc3, 1)) bogusc3])
                        ...
                        if sum(lcsvd.S) > 0
                            ...
                            if isfinite(sum(coef_reg))
                                ...
                                if isa(bogusc2, Vector)
                                    ...
                                    boguspred =
                                        predict(ols, [ones(length(bogusc3)) bogusc3])
                                    # boguspred = GLM.predict(compreg, DataFrame( x1 = bogusc3))

                                else
                                    # boguspred = GLM.predict(compreg, DataFrame(bogusc3, :auto))
                                    boguspred =
                                        predict(ols, [ones(size(bogusc3, 1)) bogusc3])
